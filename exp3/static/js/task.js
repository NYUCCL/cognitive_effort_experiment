...
// VERSION 2.0.0: condition is from 0 to 3, tells rule and incentive
...
var quiz = function(complete_fn) {
	...
	$('select[id=incent] option[value="incentA"]').html('$'+(1*answervalue).toFixed(2));
	$('select[id=incent] option[value="incentB"]').html('$'+(2*answervalue).toFixed(2));
	$('select[id=incent] option[value="incentC"]').html('$'+(3*answervalue).toFixed(2));
	$('select[id=incent] option[value="incentD"]').html('$'+(4*answervalue).toFixed(2));
...
var bonus_mag = function() {
	...
	displaytext(["Your score on the test was: "+testscore+" correct out of 8 test questions.",
		"Therefore, you have earned a $" + (nblue*answervalue).toFixed(2) + " bonus."]);
...
$(window).on('load', async () => {
	...
	// save answer value to condition dict instead of bonus value (10)
	conditionObj.answervalue = answervalue;
